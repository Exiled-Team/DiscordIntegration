            ...
            if (Plugin.Singleton.Config.PlayerHurt)
            {
                try
                {
                    if (ev.Attacker != null && ev.Attacker != ev.Target)
                    {
                        ...
                        {
                            if (IsSpy(ev.Attacker) || IsSpy(ev.Target) || ev.Target.Role == RoleType.Scp0492 && ev.Attacker.Side == Side.Scp)
                                return;
        ...
        public void OnSetGroup(ChangingGroupEventArgs ev)
        {
            try
            {
                if (Plugin.Singleton.Config.SetGroup)
                {
                    ...
                    ProcessSTT.SendData($"{ev.Player.Nickname} - Se le asigno el grupo: **{roleMessage}**.", HandleQueue.GameLogChannelId);
                }
            }
            catch (Exception e)
            {
